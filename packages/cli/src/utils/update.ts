...
export async function checkForUpdates(currentVersion: string) {
  try {
    ...
    const { stdout } = await execPromise("npm view @llh911001/claude-code-router version");
...
export async function performUpdate() {
  try {
    ...
    const { stdout, stderr } = await execPromise("npm update -g @llh911001/claude-code-router");
